...
> ⚠ **検証環境は「Windows ホスト + WSL2 Ubuntu」1 台に特化**。CLI は WSL2 上、Cowork は同ホストの Claude Desktop アプリ（GUI）で実施。第 2 章で出てくる `/usr/lib/wsl/lib`・`/mnt/c`・`HOME=/home/<user>`・hook が WSL2 で動く事実は**このマシン構成固有**で、Cowork が WSL に依存する意味ではない（Windows 上で Claude が hook の POSIX シェルを WSL2 に委譲した結果と推定）。**WSL 無し Windows / macOS は未検証**。一般化できるのは「hook は VM 外のホスト側で動く / env strip / hook FS ≠ VM FS / hook はホスト権限を持つ」まで。詳細は `docs/team-report.md` 冒頭「検証環境の前提」。
...
| Cowork validator | CLI より厳しい：kebab-case 強制、description 内の `${...}`/`<...>` 拒否（UserPromptExpansion は GUI marketplace 経由なら install 可・発火、§2.3bis で更新） | team-report §2.16 |
